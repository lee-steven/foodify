...
const App = () => {
  ...
  // Redirects to recipe site
  const handleRecipeClick = (event) => {
    axios.get(`https://api.spoonacular.com/recipes/${event.target.id}/information?apiKey=${API_KEY}`)
      .then(response => {
  ...
  return (
    <div>
      ...
      <h2>Recipes</h2>
      <Recipes handleRecipeClick={handleRecipeClick} recipes={recipes} />
